...
class TodaySale extends React.Component {
    constructor(props) {
        super(props);
        this.state = {
            Sale: {
    ...
    componentWillReceiveProps(props) {
        ...
        todaySale.status ? this.setState({ Sale: todaySale.todaySale.pager.today }) : null;
        orderInfo.status ? this.setState({ Order: orderInfo.orderInfo.pager.today }) : null;
        memberInfo.status ? this.setState({ Member: memberInfo.memberInfo.pager.today }) : null;
        singleSale.status ? this.setState({ SingleSale: singleSale.singleSale.pager.today }) : null;
        ...
        return (
            <div className="today-sale flex column">
                <div className="super-header flex between center">
                    ...
                    <span className="flex text">今日经营数据详情</span>
                    ...
                    <Tabs className="hm-supervisor-statistics-nav" onAction={this.changeTab} >
                        <Tabs.Item title="销售额" navStyle="alert">
                            <div className="today-main flex">
                                <span>所管辖门店今日总销售额<span className="orange m5">{Number(this.state.Sale.total).toLocaleString()}</span>元</span>
                            </div>
                        ...
                        <Tabs.Item title="订单数" navStyle="alert">
                            <div className="today-main flex">
                                <span>所管辖门店今日总订单数<span className="orange m5">{this.state.Order.total}</span></span>
                            </div>
                        ...
                        <Tabs.Item title="交易会员数" navStyle="alert">
                            <div className="today-main flex">
                                <span>所管辖门店今日总交易会员数<span className="orange m5">{this.state.Member.total}</span></span>
                            </div>
                        ...
                        <Tabs.Item title="客单价" navStyle="alert">
                            <div className="today-main flex">
                                <span>所管辖门店今日客单价<span className="orange m5">{Number(this.state.SingleSale.total).toLocaleString()}</span></span>
                            </div>
...
export default connect(mapStateToProps)(TodaySale);